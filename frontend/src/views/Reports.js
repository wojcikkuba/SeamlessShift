...
function Reports() {
        ...
        try {
            ...
            if (Array.isArray(data)) {
                const filteredRequests = data.filter(request =>
                    new Date(request.issue_date) >= start && new Date(request.issue_date) <= end
                );
    ...
    const fetchUserReplacements = async (userId, start, end) => {
        try {
            ...
            if (Array.isArray(data)) {
                const userReplacements = data.filter(replacement =>
    ...
    const handleDownloadReport = () => {
        ...
        const exportData = [
            ["Imię", "Nazwisko", "E-mail", "Data początkowa", "Data końcowa", "Liczba zgłoszonych prośb", "Liczba podjętych zastępstw", "Liczba odpowiedzi na Twoje zgłoszenia"],
            [userData.firstName, userData.lastName, userData.email, formatDate(startDate), formatDate(endDate), reportData.userRequestsCount, reportData.userReplacementsCount, reportData.answeredRequestsCount],
        ];
    ...
    useEffect(() => {
        ...
        const answeredRequests = userRequests.filter(request => request.status !== "Requested");
                    ...
                    <Col xs={12}>
                        <Card>
                            ...
                            <CardBody>
                                <Row>
                                    <Col md={6}>
                                        <Label for="startDate" className="mr-2">Data początkowa:</Label>
                                        <DatePicker
                                            id="startDate"
                                            selected={startDate}
                                    ...
                                    <Col md={6}>
                                        <Label for="endDate" className="mr-2">Data końcowa:</Label>
                                        <DatePicker
                                            id="endDate"
                                            selected={endDate}
                                        ...
                                        <Row className="mt-4">
                                            <Col md={12} className="text-center">
                                                {userReplacements.length === 0 ? (
                                                    <p>Brak danych o prośbach i zastępstwach w wybranym okresie.</p>
                                                ) : (
                                                    <p className="text-info h4">Statystyki dotyczące zastępstw</p>
                                                )}
                                            </Col>
                                        </Row>
                                        {userReplacements.length > 0 && (
